...
def main(readcsv=read_csv, method='defaultDense'):
    ...
    # train_algo = linear_model.LinearRegression(fit_intercept=False)
    ...
    print(indep_data.shape)
    # Now train/compute, the result provides the model for prediction
    train_result = train_algo.compute(indep_data, dep_data)
    # train_result = train_algo.fit(indep_data, dep_data)
    ...
    # predict_result = train_result.predict(pdata)

    # The prediction result provides prediction
    assert predict_result.prediction.shape == (pdata.shape[0], dep_data.shape[1])
    # assert predict_result.shape == (pdata.shape[0], dep_data.shape[1])
